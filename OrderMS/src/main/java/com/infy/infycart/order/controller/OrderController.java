package com.infy.infycart.order.controller;
...
import com.infy.infycart.order.dto.CartDTO;
import com.infy.infycart.order.dto.OrderDTO;
import com.infy.infycart.order.dto.SubscribedproductDTO;
import com.infy.infycart.order.service.OrderService;
...
public class OrderController {
	...
	public ResponseEntity<String> addProductsToCart(@RequestBody CartDTO cartDTO){
		SubscribedproductDTO[] subscribedproductsDTO = new RestTemplate().getForObject("http://localhost:7200/api/subscriptions/add", SubscribedproductDTO[].class);
		...
		String message = new RestTemplate().postForObject("http://localhost:7100/cart/add", cartDTO, String.class);
		return new ResponseEntity<>(message,HttpStatus.OK);
	}
	
	@PostMapping(value="/cart/delete")
	public ResponseEntity<String> deleteProductsToCart(@RequestBody CartDTO cartDTO){
		String message = new RestTemplate().postForObject("http://localhost:7100/cart/delete", cartDTO, String.class);
		return new ResponseEntity<>(message,HttpStatus.OK);
	}
}
